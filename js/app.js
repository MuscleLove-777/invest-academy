/* ============================================
   投資・資産運用 Academy - Main Application
   ============================================ */
...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('invest-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('invest-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('invest-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('invest-progress', JSON.stringify(this.progress));
        localStorage.setItem('invest-quiz-results', JSON.stringify(this.quizResults));
    },
    ...
    showDashboard() {
        ...
        var html = '<div class="fade-in">' +
            ...
            '<h2>投資・資産運用 Academy へようこそ</h2>' +
            '<p>NISA、iDeCo、株式投資、債券など資産形成の知識を体系的に学べるプラットフォームです。</p>' +
            '</div>' +
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('invest-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('invest-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('invest-progress');
            localStorage.removeItem('invest-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
